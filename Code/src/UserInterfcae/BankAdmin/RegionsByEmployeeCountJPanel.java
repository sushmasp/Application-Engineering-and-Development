...
import Business.Region.RegionEmployeeComparison;
 ...
 * @author kurki
 */
public class RegionsByEmployeeCountJPanel extends javax.swing.JPanel {

   private JPanel userProcessContainer;
   private Business business;
   
    public RegionsByEmployeeCountJPanel(JPanel userProcessContainer,Business business) {
        initComponents();
    ...
    private void populatetable()
    {
        ...
        {
           Collections.sort(business.getRegionDirectory().getRegionList(),new RegionEmployeeComparison());
           for(Region r: business.getRegionDirectory().getRegionList())
              {
                 Object[] row = new Object[2];
                 row[0]= r.getRegionName();
                 row[1] =r.getEmployeeCount();
                 dtm.addRow(row);  
               }
        }
    }
    ...
    private void initComponents() {
        ...
        tblRegions.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Region Name", "Number Of Employees"
            }
        ) {
        ...
        jLabel1.setText("Regions By Employee Count");
    ...
    private void btnGraphicalViewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnGraphicalViewActionPerformed
       DefaultCategoryDataset dataset = new DefaultCategoryDataset();
        if(!(business.getRegionDirectory().getRegionList().isEmpty()))
        {
            for (Region r : business.getRegionDirectory().getRegionList())
            {
                dataset.setValue(r.getEmployeeCount(), "Employee Count", r.getRegionName());
            }
        }
        JFreeChart chart = ChartFactory.createBarChart("Employee Count Graph", "Region", "Employee Count", dataset,PlotOrientation.VERTICAL,false,true,false);
